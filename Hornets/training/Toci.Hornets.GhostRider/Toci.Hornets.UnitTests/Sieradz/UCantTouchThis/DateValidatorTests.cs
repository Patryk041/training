...
namespace Toci.Hornets.UnitTests.Sieradz.UCantTouchThis
{
    ...
    public class DateValidatorTests
    {
        ...
        public void UCTTDateValidatorTest()
        {
            ...
            correctDatesList.ForEach(date => Assert.IsTrue(UCTT_DateValidatorUtils.IsDateValid(date[0], date[1], date[2])));
            
            incorrectDatesList.ForEach(date => Assert.IsFalse(UCTT_DateValidatorUtils.IsDateValid(date[0], date[1], date[2])));
            time.Stop();
